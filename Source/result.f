...
c     Print connectivity
     ...
     $           .and.(lrefn(ielem,1).ne.309) ! skip surface elements
     $           .and.(lrefn(ielem,1).ne.310)
     $           .and.(lrefn(ielem,1).ne.312)) then ! skip surface elements
           nen = lrefn(ielem,2)
...
c          write(12,*)8,(lnods(ielem,inode)-1,inode=1,8)
     ...
     $            .or.(lrefn(ielem,1).eq.309) ! count the surface elements
     $            .or.(lrefn(ielem,1).eq.310)
     $            .or.(lrefn(ielem,1).eq.312)) then ! count the surface elements
...
c            write(12,112)primal(ipoin,idofn) ! if print each newton iteration
          enddo
        enddo
...
c       write the gradient for nodal data 
     ...
     $    .and.(lrefn(ielem,1).ne.312)) then! skip surface elements                   
            ...
            if ((lrefn(ielem,1).eq.608).or.(lrefn(ielem,1).eq.606)) then! write x more times the value
              ...
              if (nen.eq.4) then
                nen=2-1
              elseif (nen.eq.3) then     ! triangular element LD
                nen=3-2
     ...
     $        (lrefn(ielem,1).ne.309).and. ! skip surface elements
     $        (lrefn(ielem,1).ne.310).and.
     $        (lrefn(ielem,1).ne.312))then! skip surface elements
